...
void change_bytes(int fd_patch, int fd1, int fd2) {
	...
	// Else, write over it with new_byte
	uint8_t f1_buff;
	...
	while((read_res = read(fd_patch, &record, sizeof(struct structure_1_record))) < 0) {
		...
		// Now, lseek one positionback and then write 
...
int main(int argc, char* argv[]) {
	...
	// The offset should be scaled in accordance with the data version as well
